...
#[test]
fn exported_vibesteroids_frame_changes_after_simulation_ticks() {
    const ASTEROIDS_WAT: &str = include_str!("../plugins/vibesteroids.wat");
    let mut frontplane =
        gpui_wasm::Frontplane::from_wat(ASTEROIDS_WAT, gpui_wasm::Limits::default()).unwrap();
    ...
    assert!(before.contains("VIBESTEROIDS"));
    assert!(after.contains("VIBESTEROIDS"));
}
